        ...
        lifecycleScope.launch {
            ...
            usuarioRepository.habilitados.collectLatest { usuarios ->
                val adapter = UsuarioSelectorAdapter(usuarios) { usuarioSeleccionado ->
                    ...
                    val intent = if (usuarioSeleccionado.esAdmin) {
                        // Si es admin, va a la pantalla de Login (para poner su contraseña)
                        mostrarDialogoValidacionAdmin(NewProductActivity::class.java)
                    } else {
                        // Si NO es admin, va directo a la selección de productos
                        Intent(this@SelectUserActivity, SelectProductActivity::class.java)

                        intent.putExtra("user_id", usuarioSeleccionado.id)
                        startActivity(intent)
                    }
                }
